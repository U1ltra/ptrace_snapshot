...
int main() {
    ...
    unsigned char *user_buffer2 = malloc(SNAPSHOT_SIZE);  // Buffer for PTRACE_GETSNAPSHOT data
    ...
    if (child_pid == 0) {
        ...
        if (WIFSTOPPED(status)) {
            ...
            // Perform PTRACE_GETSNAPSHOT to read the snapshot (v1)
            ...
            printf("Parent: Snapshot data read back (v1):\n");
            for (int i = 0; i < SNAPSHOT_SIZE; i++) {
            ...
            ret = ptrace(PTRACE_GETSNAPSHOT, child_pid, target_addr, (unsigned long)user_buffer2);
            if (ret == -1) {
                perror("ptrace GETSNAPSHOT");
                exit(EXIT_FAILURE);
            }
            ...
            for (int i = 0; i < SNAPSHOT_SIZE; i++) {
                printf("%02x ", user_buffer2[i]);
                if ((i + 1) % 16 == 0)
                    printf("\n");
            }

            // compare the difference between the two snapshots and print the difference
            printf("Parent: Snapshot data difference:\n");
            for (int i = 0; i < SNAPSHOT_SIZE; i++) {
                // compute the value difference between the two snapshots
                int diff = user_buffer2[i] - user_buffer[i];
                printf("%02x ", diff);
                if ((i + 1) % 16 == 0)
                    printf("\n");
            }
